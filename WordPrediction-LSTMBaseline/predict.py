...
#function to display predicted words
def predict_and_display(prompt, model, tokenizer, device, num_words=3, seq_len=10):
    ...
    for _ in range(num_words):
        ...
        generated = torch.cat([generated, next_token_id.to(device)], dim=1)
    ...
    print(f"{'LSTM Baseline':<20} {prompt:<30} {output_text:<50} {top_3_display}")

if __name__ == '__main__':
    config_path = os.path.join(os.path.dirname(__file__), "flatconfig.yaml")
    config = load_config(config_path)

    checkpoint_path = os.path.join("outputs", "visualizations", "best_lstm.pth")

    tokenizer = BertTokenizer.from_pretrained("bert-base-uncased")
    vocab_size = len(tokenizer.get_vocab()) + 1

    device = setup_device()
    model = load_model(checkpoint_path, vocab_size, config, device)
